...
export class PocketCollectionsService {
  ...
  constructor(
    ...
    this.countChamadosWithStatus('em_espera').subscribe((count) => {
      this.chamados_em_espera = count;
      this.em_espera_event.next(count);
    });

    this.countChamadosWithStatus('em_andamento').subscribe((count) => {
      this.chamados_em_andamento = count;
      this.em_andamento_event.next(count);
    });

    this.countChamadosWithStatus('aguardando').subscribe((count) => {
      this.chamados_aguardando = count;
      this.aguardando_event.next(count);
    });
  }

  salvarDescricao(id: string, content: string) {
    this.pb.collection('chamados').update(id, { description: content });
  }

  salvarTitulo(id: string, title: string) {
    this.pb.collection('chamados').update(id, { title: title });
  }

  getChamadosWithMultipleStatus(
    status: string[],
    filterExtra = '',
    ...
    this.pb
      .collection('chamados')
      .getList(1, 50, {
        filter: `${statusFilters.join(' || ')}` + filterExtra,
    ...
    this.chamadosEvent.subscribe((e) => {
      ...
      if (action === 'create') {
        if (status.includes(record['status'])) chamados.push(record);
      } else if (action === 'delete') {
        const idToRemove = record.id;
        const indexToRemove = chamados.findIndex(
          (chamado) => chamado.id === idToRemove,
        );
        ...
        if (!status.includes(record['status'])) {
          ...
          let findIndex = chamados.findIndex(
            (chamado) => chamado.id === record.id,
          );
          ...
          const indexToUpdate = chamados.findIndex(
            (chamado) => chamado.id === idToUpdate,
          );
  ...
  getChamadosWithStatus(
    ...
    this.pb
      .collection('chamados')
      .getList(1, 50, {
        filter: `status = '${status}'` + filterExtra,
        requestKey: 'chamadosstatus',
        expand: 'users, created_by',
      })
      .then((res) => {
    ...
    this.chamadosEvent.subscribe((e) => {
      ...
      if (action === 'create') {
        if (record['status'] == status) chamados.push(record);
      } else if (action === 'delete') {
        const idToRemove = record.id;
        const indexToRemove = chamados.findIndex(
          (chamado) => chamado.id === idToRemove,
  ...
  getChamadosEmpty(): Observable<RecordModel[]> {
    ...
    this.pb
      .collection('chamados')
      .getFullList({ filter: 'users:length = 0' })
      .then((chamados) => {
        chamadoSubject.next(chamados);
      });
  ...
  countChamadosWithStatus(status: string): Observable<Number> {
    ...
    pb.collection('chamados')
      .getList(1, 50, { filter: `status = '${status}'` })
      .then((res) => {
  ...
  _updateUserChamados(
    users: RecordModel[],
    newChamado: RecordModel,
  ): RecordModel[] {
    let updatedAndRemoved = users.map((user: RecordModel) => {
      user['chamados'] = user['chamados']
        .map((chamado: RecordModel) => {
  ...
  __addChamadoToUsers(users: RecordModel[], chamado: RecordModel) {
    users.forEach((user: RecordModel) => {
      if (chamado['users'].includes(user.id)) {
        if (user.expand) user.expand['chamados_via_users'].push(chamado);
      }
    });
  }
  ...
  getTecnicosWithChamados(): Observable<RecordModel[]> {
    ...
    this.pb
      .collection('users')
      .getFullList({
        expand: 'chamados_via_users.users, chamados_via_users.created_by',
      })
      .then((users) => {
        tecnicos = users;
    ...
    this.pb.collection('chamados').subscribe(
      '*',
      (e) => {
        console.log('Event:', e);
        if (e.action === 'create') {
          ...
          if (
            ...
            const chamado = e.record;
            const updatedAndRemoved = this._removeUserChamados(
              tecnicos,
              chamado,
            );
      ...
      {
        expand:
          'users, users.chamados_via_users.users, users.chamados_via_users.created_by',
      },
    );
  ...
  getTecnicosJoinChamados(): Observable<RecordModel[]> {
    ...
    from(this.pb.collection('users').getFullList())
      .pipe(
        switchMap((users) => {
          const userFilters = users
            .map((user) => `users.id?="${user.id}"`)
            .join('||');
          return from(
            this.pb.collection('chamados').getFullList({
              filter:
                '(' +
                userFilters +
                ')' +
          ...
          ).pipe(map((chamados: RecordModel[]) => ({ users, chamados })));
        }),
      )
      .subscribe({
        next: ({ users, chamados }) => {
          ...
          chamados.forEach((chamado: RecordModel) => {
            chamado['users'].forEach((userID: any) => {
              const userFound = usersWithChamados.find(
                (user: RecordModel) => user.id === userID,
              );
              if (userFound) {
    ...
    this.chamadosEvent.subscribe((e) => {
      ...
      if (action === 'update' || action === 'create') {
        if (
          ...
          usersWithChamadosR = this._removeUserChamados(
            usersWithChamadosR,
            record,
          );
        } else {
          let recordExpanded = this._expandUsers(record, usersWithChamadosR);
          usersWithChamadosR = this._updateUserChamados(
            usersWithChamadosR,
            record,
          );
        }
  ...
  getRelatoriosParsed(
    ...
    this.pb
      ...
      .getFullList({
        filter: `chamado = '${chamado_id}' ${extra_filter || ''}`,
        sort: '-created',
        expand: 'user',
      })
      .then((item) => {
  ...
  startUserTimer(
    ...
    if (!this.pb.authStore.isValid) return of(false);
    ...
    this.apiSrv.GetTime().subscribe((time) => {
      ...
      let promise = this.pb
        .collection('horas')
        .create(data, { requestKey: 'startchamado' });
      promise.then((hora) => {
  ...
  pauseUserTimer(
    ...
    this.apiSrv.GetTime().subscribe((time) => {
      this.pb
        .collection('horas')
        .getFirstListItem(
          `chamado.id = '${chamado_id}' && user.id = '${userId}' && end_time = ''`,
          { requestKey: 'pausechamado' },
        )
        .then((hora) => {
          let promise = this.pb.collection('horas').update(
            hora.id,
            {
              end_time: time,
            },
  ...
  updateChamadoUsers(chamadoID: string, userIds: string[]) {
    ...
    this.http
      .post(environment.apiUrl + '/updateUsersChamado', postData)
      .subscribe((data: any) => {
  ...
  getChamadoUsersFinalized(chamado_id: string): Observable<any> {
    ...
    this.pb
      .collection('user_finalizados')
      .getFullList({ filter: `chamado.id = '${chamado_id}'` })
      .then((item) => {
  ...
  getChamadoTimers(chamado_id: string): Observable<any> {
    ...
    this.pb
      .collection('duracao_chamados')
      .getFullList({ filter: `chamado.id = '${chamado_id}'`, expand: 'user' })
      .then((item) => {
    ...
    this.horasEvent.subscribe((e) => {
      this.pb
        ...
        .getFullList({
          ...
          expand: 'user',
          requestKey: 'GEtchamadostimerskey',
        })
        .then((item) => {
          timersSubject.next(item);
        });
  ...
  getRelatorioSketch(chamado_id: string): Observable<RecordModel> {
    return from(
      this.pb
        .collection('relatorio_sketchs')
        .getFirstListItem(
          `chamado.id = '${chamado_id}' && user.id = '${this.pb.authStore.model!['id']}'`,
        ),
    );
  }

  saveRelatorioSketch(chamado_id: string, html: string) {
    this.getRelatorioSketch(chamado_id)
      .pipe(
        tap((sketch) => {
