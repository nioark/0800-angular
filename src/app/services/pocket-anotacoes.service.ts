...
export class PocketAnotacoesService {
  ...
  fetchPublicBlocos() : Observable<RecordModel[]>{
    ...
    this.pb.collection('blocos').getFullList({filter: "public = true", expand: "anotacoes_via_bloco.bloco"}).then((data) => {
      blocos = data
      subject.next(data)
    })
  ...
  getBlocosOwnAnotacoes(): Observable<RecordModel[]>{
    ...
    this.pb.collection('blocos').getFullList({expand: 'anotacoes_via_bloco.bloco', filter: "created_by = " +  "'" + this.pb.authStore.model!["id"] + "' && public = false"}).then((blocosData) => {
      blocos = blocosData
      subject.next(blocos)
    })
  ...
  async addUserAnotacao(id_user : string, titulo : string, descricao : string){
    let bloco = await this.pb.collection('blocos').getFullList({filter: `created_by = '${id_user}' && public = true`})
  ...
  async copiarAnotacao(id_anotacao : string, id_bloco : string){
    ...
    let copia = await this.addAnotacaoAsync(id_bloco, anotacao["titulo"], anotacao["descricao"])
    const formData = new FormData();


    for (let i = 0; i < anotacao["imagens"].length; i++){
     let link_image = environment.apiUrl + "/api/files/anotacoes/" + id_anotacao + "/" + anotacao["imagens"][i]
