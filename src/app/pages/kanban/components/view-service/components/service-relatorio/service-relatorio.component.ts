...
export class ServiceRelatorioComponent {
  ...
  editorCarregado(){
    this.editorNew!.editor.setContent(this.html as string);
     ...
     start_time.value = this.getTimeFromDate(this.relatorio_data["interacao_start"])
  ...
  getTimeFromDate(date : string) : string {
    ...
    let timestring = dateDate.toLocaleTimeString('pt-br').split(' ')[0].split(':').slice(0,2).join(':');
    return timestring;
  }
